...
class ConversationModel(models.Model):
    id = models.CharField(max_length=255, primary_key=True, verbose_name='대화 ID')
    table1_id = models.CharField(max_length=50, verbose_name='테이블1 ID', db_index=True)
    table2_id = models.CharField(max_length=50, verbose_name='테이블2 ID', db_index=True)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='생성일시')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['table1_id', 'table2_id'], name='idx_table_ids'),
        ]
    
    def __str__(self):
        return f"Conversation {self.id} - {self.table1_id} <-> {self.table2_id}"


class MessageModel(models.Model):
    conversation = models.ForeignKey(ConversationModel, on_delete=models.CASCADE, verbose_name='대화', db_index=True)
    from_table_id = models.CharField(max_length=50, verbose_name='발신 테이블 ID', db_index=True)
    to_table_id = models.CharField(max_length=50, verbose_name='수신 테이블 ID', db_index=True)
    content = models.TextField(verbose_name='메시지 내용')
    timestamp = models.DateTimeField(auto_now_add=True, verbose_name='전송 시각', db_index=True)
    ...
    class Meta:
        ...
        ordering = ['-timestamp']
        indexes = [
            models.Index(fields=['conversation', 'timestamp'], name='idx_conversation_timestamp'),
            models.Index(fields=['from_table_id', 'to_table_id', 'timestamp'], name='idx_table_timestamp'),
        ]
    
    def __str__(self):
        return f"Message from {self.from_table_id} to {self.to_table_id}: {self.content[:50]}"


class BlockedTableModel(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['blocker_table_id'], name='idx_blocker'),
        ]
    
    def __str__(self):
        return f"{self.blocker_table_id} blocked {self.blocked_table_id}"


class PushSubscriptionModel(models.Model):
    table_id = models.CharField(max_length=50, primary_key=True, verbose_name='테이블 ID')
    ...
    def __str__(self):
        return f"Push subscription for table {self.table_id}"